                            ...
                            if 0 <= credit_differ <= 120 and credit_differ % 20 == 0:
                                ...
                                if 0 <= credit_fail <= 120 and credit_fail % 20 == 0:
                                    ...
                                    if total != 120:
                                        ...
                                        data_dictionary[stu_id] = f'Progress -{credit_pass}, {credit_differ},{credit_fail}'  #add marks to dictionary
                                    elif credit_pass == 100:
                                        print("Progress (module trailer)")
                                        trailer_num = trailer_num + 1
                                        data_dictionary[stu_id] = f'Progress (module trailer) -{credit_pass}, {credit_differ},{credit_fail}'
                                    elif 0 <= credit_pass <= 80 and 0 <= credit_differ <= 120 and 0 <= credit_fail <= 60:
                                        print("Do not progress – module retriever")
                                        retriever_num = retriever_num + 1
                                        data_dictionary[stu_id] = f'Module retriever -{credit_pass}, {credit_differ},{credit_fail}'
                                    elif 0 <= credit_pass <= 40 and 0 <= credit_differ <= 40 and 80 <= credit_fail <= 120:
                                        print("Exclude")
                                        excluded_num = excluded_num + 1
                                        data_dictionary[stu_id] = f'Exclude -{credit_pass}, {credit_differ},{credit_fail}'
...
def histogram(progress_num, trailer_num, excluded_num, retriever_num):     #create a function called 'histo' for histogram
    while True:
        ...
        if choose == "y":
            ...
            print("-" * 64)
            print("Part 4:")
            print(str(data_dictionary).replace('{', '').replace("'", '').replace('}', ''))  #https://stackoverflow.com/questions/29459008/how-to-remove-brackets-from-python-string
            exit()
        else:
